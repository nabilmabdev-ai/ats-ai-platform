  ...
  async process(job: Job<any>) {
    if (job.name === 'reindex-candidate') {
      ...
      try {
        ...
        console.log(`✅ Re-indexed candidate ${candidateId} in Milvus`);

        // Sync to MeiliSearch
        await this.searchService.indexCandidate(candidate);
        console.log(`✅ Re-indexed candidate ${candidateId} in MeiliSearch`);
      } catch (e: any) {
        console.error(
    ...
    try {
      ...
      const newStatus: AppStatus = application.status;
      ...
      try {
        ...
        console.error('Vectorization/Indexing Failed:', e);
      }
