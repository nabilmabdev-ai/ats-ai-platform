...
async function debugSourcing() {
    ...
    console.log(`   Description length: ${job.descriptionText?.length || 0} chars`);
  ...
  `.trim();
    ...
    if (matches.length === 0) {
        console.warn('⚠️ No matches found by AI. Check if embeddings exist for candidates.');
        return;
    }
    ...
    for (const c of candidates) {
        const hasRejection = c.applications.some(app => app.status === AppStatus.REJECTED);
        const isHired = c.applications.some(app => app.status === AppStatus.HIRED);
        ...
        console.log(`   - Candidate ${c.id.substring(0, 8)} (${c.firstName} ${c.lastName})`);
        console.log(`     Has Rejection: ${hasRejection} | Is Hired: ${isHired} | Applied to this: ${appliedToThisJob}`);
