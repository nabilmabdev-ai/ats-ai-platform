...
*   **📋 Pipeline Management**: Drag-and-drop Kanban board for candidate tracking with "Backward Move" support and stage-based filtering.
*   **🤝 Offer Management**: Comprehensive offer workflow including approval chains, document generation, and status tracking.
*   **📅 Smart Scheduling**: Calendar conflict detection considering candidate timezones, interviewer work hours, and Google Calendar busy periods (via React Big Calendar).
...
*   **🛡️ Enterprise Stability**: Global Error Boundaries (Frontend), Centralized Exception Handling (Backend), and Standardized Logging.
...
| :--- | :--- | :--- | :--- | :--- |
| **Frontend** | `/apps/frontend` | Next.js 16 (App Router), Tailwind v4, SWR | 3000 | User Interface & Dashboards. |
| **Backend Core** | `/apps/backend-core` | NestJS 11, Prisma, BullMQ | 3001 | Business Logic, API, Auth, SQL DB. |
...
# This runs the main seed script (foundation, jobs, candidates, interviews)
npm run seed

# Optional Granular Seeding:
# npm run seed:foundation  # Users, Roles, Settings
# npm run seed:jobs        # Job Templates & Listings
# npm run seed:candidates  # Candidates & Applications
# npm run seed:ops         # Interviews & Offers
...
4.  Select the folder: `apps/chrome-extension`.
5.  **Usage**: Go to a LinkedIn profile -> Click Extension Icon -> Enter Email -> Click "Import Profile".

## 🧪 Development & Seeding

The system comes with a robust seeder to populate the database with realistic test data (Jobs, Candidates, Applications, Interviews, Offers).
...
1.  **Keyword Query**: Sent to MeiliSearch to find exact string matches (skills, names).
2.  **Semantic Query**: Sent to Backend-AI to vectorize the query (via Gemini) and search Milvus for conceptual matches.
3.  **Fusion**: The two result lists are merged. Candidates appearing in both lists get a boosted score.
